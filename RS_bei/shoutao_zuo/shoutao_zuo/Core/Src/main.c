...
/* USER CODE BEGIN PV */
const uint8_t poll_channels[] = {0, 1, 2, 3};  // 要轮询的通道
#define CHANNEL_COUNT (sizeof(poll_channels) / sizeof(poll_channels[0]))  // 通道数量（4个）
int change_flag = 0; 
...
//			mpu_data[3].filter_angle_x,mpu_data[2].filter_angle_x,
//			mpu_data[1].filter_angle_x,mpu_data[0].filter_angle_x, 
//			jy901s_data.angle_roll,jy901s_data.angle_pitch);
//		}
		if(trans_flag == 1 && allow_send_flag ==1 )
		{
			switch (current_cmd) 
        {
						case CMD_NONE:    																				        																break;
...
//		if(trans_flag == 1 && allow_send_flag ==0 )
//		{
//			control_cnt++;
//			if(control_cnt > 5)
//			{
...
//					HAL_Delay(10);
		...
		// 通道计数器递增，自动循环
		channel_cnt++;		
//		HAL_UART_Transmit(&huart1, &current_cmd, sizeof(current_cmd), 0xff);
    /* USER CODE END WHILE */
