 ...
 * 3. 它会按 100ms 节拍更新一次“最新样本”，再按 200ms 节拍执行一次分类和命令过滤。
 */
 ...
 * 字段说明：
 *   sample_updated: 这次输入是否刷新了 100ms 样本缓存。
 *   classify_ready: 这次是否真的执行了一次分类。
 *   send_ready: 这次分类结果是否已经通过发送过滤，可以直接发命令。
...
/*
 * 函数作用：
 *   持续喂入一帧已经解包完成的手套数据，并在需要时更新 100ms 样本缓存。
 * 调用时机：
 *   应用层每收到一帧合法协议数据后调用一次。
 * 参数说明：
 *   p_frame: 当前收到的合法协议帧。
...
/*
 * 函数作用：
 *   在 200ms 调度节拍到来时，使用最新样本执行一次 SVM 分类和命令过滤。
 * 调用时机：
 *   只有当应用层确认已经到了一个新的 200ms 周期时才调用。
 * 参数说明：
 ...
 * 调用方式：
 *   如果返回后 p_result->send_ready 为 true，应用层才应该真的发送命令字节。
 */
